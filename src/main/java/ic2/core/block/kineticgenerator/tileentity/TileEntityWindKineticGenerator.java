...
public class TileEntityWindKineticGenerator extends TileEntityAbstractKineticGenerator
  ...
  public static final float outputModifier =
      10.0F * (float) IC2Config.balance.energy.kineticGenerator.wind.get().floatValue();
      ...
      if (nextActive) {
        ...
        if (this.obstructedCrossSection < 0) {
          ...
          float speed =
              (float)
                  Util.limit(
                      (this.windStrength - this.getMinWindStrength()) / this.getMaxWindStrength(),
                      0.0,
                      2.0);
        ...
        for (int fwd = lentemp - length; fwd <= length; fwd++) {
          ...
          if (!state.isAir()) {
            ...
            if ((up != 0 || right != 0 || fwd != 0)
                && chunkCache.getBlockEntity(pos) instanceof TileEntityWindKineticGenerator
                && !onlyRotor) {
              return -1;
            }
          }
