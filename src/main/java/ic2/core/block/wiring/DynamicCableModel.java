...
public abstract class DynamicCableModel<T, E> implements UnbakedModel, BakedModel {
  ...
  /**
   * {@return whether this cable is covered in construction foam}
   *
   * <p>{@code null} means the model JSON asked for {@code "foam": "none"}, i.e. a bare cable. {@link
   * CableFoam} only models the soft and hard (coloured) foams, so absence is represented here rather
   * than by an enum constant.
   */
  protected boolean hasFoam() {
    return this.foam != null && this.foam.isPresent();
  }
