...
public class LaserBulletEntity extends ThrowableProjectile {
  ...
  private boolean hitBlock(BlockPos pos, Direction side) {
    ...
    // NeoForge's BlockEvent.BreakEvent is a vanilla-side hook; on Fabric the equivalent veto for a
    // player-caused break is the interaction event, which is server-side only.
    if (!world.isClientSide
        && !PlayerBlockBreakEvents.BEFORE
            .invoker()
            .beforeBlockBreak(world, playerOwner, pos, state, world.getBlockEntity(pos))) {
      this.remove(RemovalReason.DISCARDED);
      return true;
    }
    ...
    if (block == Blocks.TNT) {
      ...
      if (IC2.envProxy.isFlammable(state, world, pos, side)) {
    ...
    if (event.removeBlock) {
      ...
      if (world.random.nextInt(10) == 0 && IC2.envProxy.isFlammable(state, world, pos, Direction.UP)) {
