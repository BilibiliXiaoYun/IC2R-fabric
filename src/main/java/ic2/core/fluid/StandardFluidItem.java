...
public interface StandardFluidItem extends Ic2FluidItem {
  ...
  default Ic2FluidStack drainMb(
    ...
    if (stack.getCount() != 1) {
      // IC2 only supports fluid operations on one container at a time. External fluid handlers may
      // inspect a stacked container before deciding whether they can transfer it, so a stacked
      // container reports an empty tank instead of throwing at them.
      return Ic2FluidStack.EMPTY;
    }
  ...
  default int drainMb(
    ...
    if (stack.getCount() != 1) {
      // See drainMb(ItemStack, int, boolean, Mutable): stacked containers expose nothing.
      return 0;
    }
    ...
    if (stack.isDamageableItem()) {
      ...
      if (maxDmg > 2) {
        stack.setDamageValue(
            maxDmg - 1 - (int) Util.map(amount, this.getCapacityMb(stack), maxDmg - 2));
      }
    }
  }
}
