...
public class CraftingMachineGameTests {
  ...
  // industrial workbench: the tool combo slots pair a fixed tool with one input and expose the
  // crafting result
  ...
  public static void industrialWorkbenchToolCombosResolveRecipes(GameTestHelper helper) {
    ...
    TileEntityIndustrialWorkbench te =
        getTe(helper, MACHINE_POS, TileEntityIndustrialWorkbench.class);
    ...
    helper.assertTrue(
        hammered.getItem() == Ic2Items.IRON_PLATE,
        "hammer + iron ingot should offer an iron plate, offers " + hammered);
    ...
    helper.assertTrue(
        cut.getItem() == Ic2Items.COPPER_CABLE && cut.getCount() == 2,
    ...
    helper.assertTrue(
        withoutTool.getItem() != Ic2Items.IRON_PLATE,
        "without the hammer the plate recipe must not match, offers " + withoutTool);

    helper.succeed();
  }

  // batch crafter: 40 ticks at 2 EU/t per crafting operation, pulling from the per-slot ingredient
  // buffers
  ...
  public static void batchCrafterCraftsConfiguredRecipe(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          ...
          helper.assertTrue(
              output.getItem() == Items.CRAFTING_TABLE && output.getCount() == 1,
              "the batch crafter should produce exactly one crafting table, has " + output);
          helper.assertTrue(
              te.ingredientsRow[0].get().isEmpty(), "the plank ingredients should be consumed");
        });
  }
  ...
  public static void batchCrafterWithoutEnergyDoesNotCraft(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertTrue(
              te.craftingOutput.get(0).isEmpty(),
              "an unpowered batch crafter must not produce output");
          helper.assertValueEqual(
              te.ingredientsRow[0].get().getCount(),
              1,
              "an unpowered batch crafter must not consume ingredients");
          helper.succeed();
        });
  }
  ...
  private static TileEntityBatchCrafter setupPlankCrafter(GameTestHelper helper) {
    ...
    te.matrixChange(-1);
    helper.assertTrue(
        te.recipeOutput.getItem() == Items.CRAFTING_TABLE,
        "the configured grid should resolve to a crafting table, resolves to " + te.recipeOutput);
