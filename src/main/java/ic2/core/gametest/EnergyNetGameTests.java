...
public class EnergyNetGameTests {
  ...
  private static final String TALL = "ic2:gametest/empty3x9x3";
  ...
  public static void powerTierTableMatchesClassicValues(GameTestHelper helper) {
    Ic2GameTestAssertions.assertNear(
        helper, EnergyNet.instance.getPowerFromTier(1), 32.0, "tier 1 (LV) power");
    Ic2GameTestAssertions.assertNear(
        helper, EnergyNet.instance.getPowerFromTier(2), 128.0, "tier 2 (MV) power");
    Ic2GameTestAssertions.assertNear(
        helper, EnergyNet.instance.getPowerFromTier(3), 512.0, "tier 3 (HV) power");
    Ic2GameTestAssertions.assertNear(
        helper, EnergyNet.instance.getPowerFromTier(4), 2048.0, "tier 4 (EV) power");
    helper.assertValueEqual(EnergyNet.instance.getTierFromPower(32.0), 1, "tier of a 32 EU packet");
    helper.assertValueEqual(
        EnergyNet.instance.getTierFromPower(512.0), 3, "tier of a 512 EU packet");
    helper.succeed();
  }
  ...
  public static void batboxPowersAdjacentLvMachine(GameTestHelper helper) {
    ...
    TileEntityElectricBatBox batbox =
        getTe(helper, new BlockPos(1, 2, 1), TileEntityElectricBatBox.class);
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, macerator.getEnergy(), 576.0, "macerator buffer");
  ...
  // CESU (MV, 128 EU) -> LV transformer -> LV machine: stepped-down packets arrive without an
  // explosion
  @GameTest(template = EMPTY)
  public static void lvTransformerStepsDownMvForLvMachine(GameTestHelper helper) {
    ...
    TileEntityElectricCESU cesu =
        getTe(helper, new BlockPos(1, 2, 1), TileEntityElectricCESU.class);
    ...
    helper.runAtTickTime(
        ...
        () -> {
          ...
          helper.assertTrue(
              macerator.getEnergy() >= 512.0,
              "macerator should have received stepped-down energy, has " + macerator.getEnergy());
          helper.succeed();
        });
  }
  ...
  public static void longCopperRunLosesWholeEuPerPacket(GameTestHelper helper) {
    ...
    helper.setBlock(new BlockPos(1, 0, 1), Ic2Blocks.MACERATOR);
    TileEntityElectricBatBox batbox =
        getTe(helper, new BlockPos(1, 7, 1), TileEntityElectricBatBox.class);
    TileEntityMacerator macerator = getTe(helper, new BlockPos(1, 0, 1), TileEntityMacerator.class);
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, macerator.getEnergy(), 310.0, "macerator buffer after long run");
          Ic2GameTestAssertions.assertNear(helper, batbox.energy.getEnergy(), 0.0, "batbox buffer");
        });
  }
  ...
  public static void redstoneFlipsTransformerToStepUp(GameTestHelper helper) {
    ...
    TileEntityElectricBatBox batbox =
        getTe(helper, new BlockPos(1, 2, 1), TileEntityElectricBatBox.class);
    TileEntityTransformer transformer =
        getTe(helper, new BlockPos(1, 1, 1), TileEntityTransformer.class);
    TileEntityElectricCESU cesu =
        getTe(helper, new BlockPos(1, 0, 1), TileEntityElectricCESU.class);
    ...
    helper.succeedWhen(
        () -> {
          helper.assertTrue(
              transformer.getActive(), "transformer should be in step-up mode next to redstone");
          helper.assertTrue(
              cesu.energy.getEnergy() >= 128.0,
              "CESU should have received stepped-up energy, has " + cesu.energy.getEnergy());
        });
  }
  ...
  public static void hvTransformerStepsDownEvForMfe(GameTestHelper helper) {
    ...
    helper.setBlock(
        new BlockPos(1, 1, 1),
        Ic2Blocks.HV_TRANSFORMER
            .defaultBlockState()
            .setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.UP));
    helper.setBlock(new BlockPos(1, 0, 1), Ic2Blocks.MFE);
    TileEntityElectricMFSU mfsu =
        getTe(helper, new BlockPos(1, 2, 1), TileEntityElectricMFSU.class);
    ...
    helper.runAtTickTime(
        ...
        () -> {
          ...
          helper.assertTrue(
              ...
              "MFE should have received stepped-down energy, has " + mfe.energy.getEnergy());
          helper.succeed();
        });
  }

  // bare copper absorbs 32 EU, so LV current is safe but an MV packet shocks entities next to the
  // cable
  @GameTest(template = EMPTY)
    ...
    helper.succeedWhen(
        () ->
            helper.assertTrue(
                ...
                "pig next to a live bare cable should take shock damage"));
  }

  // MFE (HV, 512 EU) -> MV transformer -> CESU (tier 2): stepped-down packets arrive without an
  // explosion
  @GameTest(template = EMPTY)
  public static void mvTransformerStepsDownHvForCesu(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertBlockPresent(Ic2Blocks.CESU, new BlockPos(1, 0, 1));
          helper.assertTrue(
              cesu.energy.getEnergy() >= 1024.0,
              "CESU should have received stepped-down energy, has " + cesu.energy.getEnergy());
          helper.succeed();
        });
  }
  ...
  public static void evTransformerStepsUpAndBackDown(GameTestHelper helper) {
    ...
    TileEntityElectricMFSU source =
        getTe(helper, new BlockPos(1, 7, 1), TileEntityElectricMFSU.class);
    TileEntityTransformer stepUp =
        getTe(helper, new BlockPos(1, 6, 1), TileEntityTransformer.class);
    TileEntityElectricMFSU sink =
        getTe(helper, new BlockPos(1, 4, 1), TileEntityElectricMFSU.class);
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertTrue(
              stepUp.getActive(), "EV transformer next to redstone should be in step-up mode");
          ...
          helper.assertTrue(
              sink.energy.getEnergy() >= 8192.0,
              "MFSU should have received twice-transformed energy, has " + sink.energy.getEnergy());
          helper.succeed();
        });
  }

  // gold carries HV; bare, insulated and double insulated segments in series lose 3 * 0.4 EU,
  // floored to 1 EU per packet
  @GameTest(template = TALL)
  public static void goldCablesCarryHvThroughAllInsulationLevels(GameTestHelper helper) {
    ...
    TileEntityElectricMFE source =
        getTe(helper, new BlockPos(1, 5, 1), TileEntityElectricMFE.class);
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, sink.energy.getEnergy(), 2044.0, "MFE buffer after the gold run");
          Ic2GameTestAssertions.assertNear(
              helper, source.energy.getEnergy(), 0.0, "source MFE buffer");
        });
  }

  // iron carries EV; all four insulation levels in series lose 4 * 0.8 EU, floored to 3 EU per
  // packet
  @GameTest(template = TALL)
  public static void ironCablesCarryEvThroughAllInsulationLevels(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, sink.energy.getEnergy(), 8180.0, "MFSU buffer after the iron run");
          Ic2GameTestAssertions.assertNear(
              helper, source.energy.getEnergy(), 0.0, "source MFSU buffer");
        });
  }
  ...
  public static void glassFibreCableCarriesHvLosslessly(GameTestHelper helper) {
    ...
    helper.setBlock(new BlockPos(1, 0, 1), Ic2Blocks.MFE);
    TileEntityElectricMFE source =
        getTe(helper, new BlockPos(1, 2, 1), TileEntityElectricMFE.class);
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, sink.energy.getEnergy(), 2048.0, "MFE buffer after the glass fibre run");
          Ic2GameTestAssertions.assertNear(
              helper, source.energy.getEnergy(), 0.0, "source MFE buffer");
        });
  }
  ...
  public static void insulatedTinCableCarriesLv(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              helper, macerator.getEnergy(), 320.0, "macerator buffer after the insulated tin run");
          Ic2GameTestAssertions.assertNear(helper, batbox.energy.getEnergy(), 0.0, "batbox buffer");
        });
  }
    ...
    helper.runAtTickTime(
        ...
        () -> {
          ...
          Ic2GameTestAssertions.assertNear(
              ...
              pig.getHealth(),
              pig.getMaxHealth(),
              "pig next to an insulated cable must not take shock damage");
          Ic2GameTestAssertions.assertNear(
              helper,
              sink.energy.getEnergy(),
              1280.0,
              "CESU buffer after the insulated copper run");
          helper.succeed();
        });
  }
    ...
    helper.succeedWhen(
        () -> {
          helper.assertBlockProperty(
              new BlockPos(1, 1, 1), AbstractDetectorCableBlock.active, true);
    ...
    helper.succeedWhen(
        () -> {
          Ic2GameTestAssertions.assertNear(
              ...
              macerator.getEnergy(),
              320.0,
              "macerator buffer behind an unpowered splitter");
          Ic2GameTestAssertions.assertNear(helper, batbox.energy.getEnergy(), 0.0, "batbox buffer");
        });
  }
  ...
  public static void redstoneCutsSplitterCableCurrent(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertBlockProperty(
              new BlockPos(1, 1, 1), AbstractSplitterCableBlock.active, true);
          Ic2GameTestAssertions.assertNear(
              helper,
              macerator.getEnergy(),
              0.0,
              "macerator must not receive energy through a powered splitter");
          Ic2GameTestAssertions.assertNear(
              helper, batbox.energy.getEnergy(), 320.0, "batbox must keep its energy");
          helper.succeed();
        });
  }
  ...
  public static void luminatorLightsOnRedstoneAndCurrent(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          ...
          helper.assertTrue(
              batbox.energy.getEnergy() < 400.0, "luminator should draw EU from the batbox");
        });
  ...
  public static void energyTransferRunsOncePerTickOnServerThread(GameTestHelper helper) {
    ...
    RecordingSink sink =
        new RecordingSink(helper.getLevel(), helper.absolutePos(new BlockPos(1, 1, 1)));
    ...
    helper.runAtTickTime(
        ...
        () -> {
          try {
            helper.assertFalse(
                sink.offThreadInjection.get(),
                "injectEnergy must only be called on the server thread");
            helper.assertTrue(
                sink.injectionsByTick.size() >= 10,
                "sink should have been powered for several ticks, got "
                    + sink.injectionsByTick.size());

            for (Map.Entry<Long, Double> entry : sink.injectionsByTick.entrySet()) {
              Ic2GameTestAssertions.assertNear(
                  helper, entry.getValue(), 1.0, "energy injected during tick " + entry.getKey());
            }
          } finally {
