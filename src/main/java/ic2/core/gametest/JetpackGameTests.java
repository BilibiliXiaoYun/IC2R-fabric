...
public class JetpackGameTests {
  ...
  // The jetpack only flies because the platform layer calls EventHandler#onPlayerTick every tick;
  // NeoForge did that through PlayerTickEvent. Fabric has no player tick event, so the call sits in
  // FabricLifecycle's world-tick hook — the regression this guards is that the hook silently stopped
  // being invoked, which no test that calls JetpackLogic directly can detect.
  @GameTest(template = EMPTY)
  public static void playerTickHookFliesEquippedJetpack(GameTestHelper helper) {
    ...
    IC2.keyboard.processKeyUpdate(player, Keyboard.Key.toInt(EnumSet.of(Keyboard.Key.jump)));
    try {
    ...
    Ic2GameTestAssertions.assertNear(
        ...
        player.getDeltaMovement().y,
        0.14,
        "the platform player-tick hook must reach JetpackHandler and lift the player");
    Ic2GameTestAssertions.assertNear(
        helper,
        ElectricItem.manager.getCharge(stack),
        MAX_CHARGE - 8.0,
        "the platform player-tick hook must drain the equipped jetpack");
    helper.succeed();
  }

  @GameTest(template = EMPTY)
  public static void forwardKeyAddsHorizontalThrust(GameTestHelper helper) {    Player player = makeAirbornePlayer(helper);
    player.setYRot(0.0F);
    ItemStack stack =
        ElectricItemManager.getCharged(Ic2Items.JETPACK_ELECTRIC, Double.POSITIVE_INFINITY);
    IJetpack jetpack = (IJetpack) stack.getItem();
