...
public class MachineGameTests {
  ...
  public static void maceratorGrindsCobblestoneToSand(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          ...
          helper.assertTrue(
              output.getItem() == Items.SAND && output.getCount() == 1,
              "macerator should produce 1 sand, has " + output);
  ...
  public static void maceratorWithoutEnergyDoesNothing(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertTrue(
              te.outputSlot.get(0).isEmpty(), "unpowered macerator must not produce output");
          helper.assertValueEqual(
              te.inputSlot.get(0).getCount(), 1, "unpowered macerator input count");
          helper.succeed();
        });
  }
  ...
  public static void electricFurnaceSmeltsCobblestone(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          ItemStack output = te.outputSlot.get(0);
          helper.assertTrue(
              output.getItem() == Items.STONE && output.getCount() == 1,
              "electric furnace should produce 1 stone, has " + output);
          helper.assertTrue(
              te.inputSlot.get(0).isEmpty(), "electric furnace input should be consumed");
        });
  }

  // induction furnace: tier 2, progress per tick scales with heat; preheated it finishes in ~13
  // ticks
  @GameTest(template = EMPTY, timeoutTicks = 200)
  public static void inductionFurnaceSmeltsBothSlotsWhenHot(GameTestHelper helper) {
    ...
    helper.succeedWhen(
        () -> {
          ...
          helper.assertTrue(
              outputA.getItem() == Items.STONE && outputA.getCount() == 1,
              "induction slot A should produce 1 stone, has " + outputA);
          helper.assertTrue(
              outputB.getItem() == Items.GLASS && outputB.getCount() == 1,
              "induction slot B should produce 1 glass, has " + outputB);
        });
  }
  ...
  public static void inductionFurnaceBuildsAndKeepsHeat(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertTrue(
              te.heat > 0, "induction furnace should build up heat while working, heat=" + te.heat);
          helper.succeed();
        });
  }
  ...
  public static void maceratorProgressSurvivesSaveAndLoad(GameTestHelper helper) {
    ...
    helper.runAtTickTime(
        ...
        () -> {
          helper.assertTrue(
              te.getProgress() > 0.5F,
              "macerator should be over half done before the reload, progress " + te.getProgress());
          CompoundTag nbt = te.saveWithFullMetadata(helper.getLevel().registryAccess());
          helper.setBlock(MACHINE_POS, Blocks.AIR);
          helper.setBlock(MACHINE_POS, Ic2Blocks.MACERATOR);
          getMachine(helper, TileEntityMacerator.class)
              .loadWithComponents(nbt, helper.getLevel().registryAccess());
        });
    ...
    helper.runAtTickTime(
        ...
        () -> {
          ...
          helper.assertTrue(
              restored.getProgress() > 0.5F,
              "macerator lost its progress on reload, progress " + restored.getProgress());
        });

    // with ~200 ticks of saved progress retained, the remaining ~100 ticks finish well within the
    // timeout
    helper.succeedWhen(
        () -> {
          TileEntityMacerator restored = getMachine(helper, TileEntityMacerator.class);
          ...
          helper.assertTrue(
              output.getItem() == Items.SAND && output.getCount() == 1,
              "macerator should produce 1 sand after the reload, has " + output);
          helper.assertTrue(
              restored.inputSlot.get(0).isEmpty(),
              "macerator input should be consumed after the reload");
        });
  }
