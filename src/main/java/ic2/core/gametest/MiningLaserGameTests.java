...
public class MiningLaserGameTests {
  ...
  public static void laserShootEventCanCancelShot(GameTestHelper helper) {
    ...
    Runnable unregister = listener.register();
    try {
      laser().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
  ...
  private static class LaserEventCancelListener {
    ...
    /** Subscribes to IC2's own laser events; NeoForge's bus has no Fabric equivalent. */
    Runnable register() {
      Runnable shooter =
          Ic2EventBus.INSTANCE.register(LaserEvent.LaserShootEvent.class, this::onShoot);
      Runnable hitBlock =
          Ic2EventBus.INSTANCE.register(LaserEvent.LaserHitsBlockEvent.class, this::onHitBlock);
      return () -> {
        shooter.run();
        hitBlock.run();
      };
    ...
    private void onShoot(LaserEvent.LaserShootEvent event) {
    ...
    private void onHitBlock(LaserEvent.LaserHitsBlockEvent event) {
  ...
  /**
   * Vetoes the laser's block break.
   *
   * <p>NeoForge delivered this as {@code BlockEvent.BreakEvent}. {@code LaserBulletEntity} now asks
   * Fabric's {@code PlayerBlockBreakEvents.BEFORE}, so the test vetoes through the same event.
   */
  private static class BlockBreakCancelListener {
    private final Player owner;
    private boolean sawBreak;
    private boolean active = true;
    private PlayerBlockBreakEvents.Before handler;
    ...
    void register() {
      // The handler is built here rather than in a field initialiser, so it can read the final
      // owner field without tripping Java's definite-assignment rules.
      this.handler =
      ...
      PlayerBlockBreakEvents.BEFORE.register(this.handler);
    }

    /**
     * Deactivates the veto.
     *
     * <p>NeoForge's event bus could remove a listener; Fabric's {@code Event} has no {@code
     * unregister}, so the handler stays registered but becomes inert once the test is done.
     */
    void unregister() {
      this.active = false;
    }
