...
public class NanoSuitGameTests {
  ...
  public static void nanosuitAbsorbsDamageAndDrainsCharge(GameTestHelper helper) {
    ...
    Ic2GameTestAssertions.assertNear(
        helper, remaining, 10.0 - 10.0 * 0.4 * 0.9, "damage left after absorption");
    Ic2GameTestAssertions.assertNear(
        helper,
        ElectricItem.manager.getCharge(chest),
        MAX_CHARGE - 10.0 * 0.4 * 0.9 * ENERGY_PER_DAMAGE,
        "charge after absorption");
  ...
  public static void nanosuitAbsorptionLimitedByCharge(GameTestHelper helper) {
    ...
    Ic2GameTestAssertions.assertNear(
        helper, ElectricItem.manager.getCharge(chest), 0.0, "charge after absorption");
  ...
  public static void nanosuitIgnoresInvulnerabilityBypassingDamage(GameTestHelper helper) {
    ...
    for (DamageSource source : sources) {
      ...
      Ic2GameTestAssertions.assertNear(
          helper, remaining, 10.0, "damage left after " + source.getMsgId());
      Ic2GameTestAssertions.assertNear(
          helper,
          ElectricItem.manager.getCharge(chest),
          MAX_CHARGE,
          "charge after " + source.getMsgId());
    }
  ...
  public static void nanobootsAbsorbShortFallsOnly(GameTestHelper helper) {
    ...
    Ic2GameTestAssertions.assertNear(
        ...
        MAX_CHARGE - 3 * ENERGY_PER_DAMAGE,
    ...
    Ic2GameTestAssertions.assertNear(
        ...
        ElectricItem.manager.getCharge(boots),
        MAX_CHARGE - 3 * ENERGY_PER_DAMAGE,
        "unabsorbed fall must not drain charge");
  ...
  public static void nanochestArmorBonusRequiresCharge(GameTestHelper helper) {
    ...
    Ic2GameTestAssertions.assertNear(
        helper, getArmorBonus(uncharged, EquipmentSlotGroup.CHEST), 0.0, "uncharged chest armor");

    ItemStack charged =
        ElectricItemManager.getCharged(Ic2Items.NANO_CHESTPLATE, Double.POSITIVE_INFINITY);
    Ic2GameTestAssertions.assertNear(
        helper,
        getArmorBonus(charged, EquipmentSlotGroup.CHEST),
