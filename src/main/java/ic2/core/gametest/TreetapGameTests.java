...
public class TreetapGameTests {
  ...
  private static void placeWetLog(GameTestHelper helper) {
    helper.setBlock(
        ...
        Ic2Blocks.RUBBER_LOG
            .defaultBlockState()
            .setValue(RubberLogBlock.stateProperty, RubberLogBlock.RubberWoodState.wet_north));
  }
  ...
  public static void treetapExtractsResinFromWetLog(GameTestHelper helper) {
    ...
    InteractionResult result =
        stack.getItem().useOn(Ic2GameTestUtil.useOn(helper, player, LOG_POS, Direction.NORTH));

    helper.assertValueEqual(result, InteractionResult.SUCCESS, "treetap use result");
    helper.assertValueEqual(
        getLogState(helper), RubberLogBlock.RubberWoodState.dry_north, "log state after tapping");
    helper.assertValueEqual(
        player.getMainHandItem().getDamageValue(), 1, "treetap durability used");
  ...
  public static void treetapWrongSideDoesNothing(GameTestHelper helper) {
    ...
    InteractionResult result =
        stack.getItem().useOn(Ic2GameTestUtil.useOn(helper, player, LOG_POS, Direction.EAST));

    helper.assertValueEqual(result, InteractionResult.FAIL, "treetap use result");
    helper.assertValueEqual(
        getLogState(helper), RubberLogBlock.RubberWoodState.wet_north, "log must stay wet");
    helper.assertValueEqual(
        player.getMainHandItem().getDamageValue(), 0, "treetap must not take damage");
    helper.succeed();
  }
  ...
  public static void treetapPlainLogDoesNothing(GameTestHelper helper) {
    helper.setBlock(
        ...
        Ic2Blocks.RUBBER_LOG
            .defaultBlockState()
            .setValue(RubberLogBlock.stateProperty, RubberLogBlock.RubberWoodState.plain));
    ServerPlayer player = makePlayer(helper);
    ItemStack stack = new ItemStack(Ic2Items.TREETAP);
    player.setItemInHand(InteractionHand.MAIN_HAND, stack);
    ...
    helper.assertValueEqual(result, InteractionResult.FAIL, "treetap use result");
    helper.assertValueEqual(
        getLogState(helper), RubberLogBlock.RubberWoodState.plain, "log must stay plain");
    helper.succeed();
  }
  ...
  public static void electricTreetapExtractsResin(GameTestHelper helper) {
    ...
    ServerPlayer player = makePlayer(helper);
    ItemStack stack =
        ElectricItemManager.getCharged(Ic2Items.ELECTRIC_TREETAP, Double.POSITIVE_INFINITY);
    player.setItemInHand(InteractionHand.MAIN_HAND, stack);
    ...
    helper.assertValueEqual(
        getLogState(helper), RubberLogBlock.RubberWoodState.dry_north, "log state after tapping");
    helper.assertValueEqual(
        ElectricItem.manager.getCharge(stack), 10000.0 - 50.0, "charge after tapping");
    helper.succeedWhen(() -> helper.assertItemEntityPresent(Ic2Items.RESIN, LOG_POS, 2.0));
  }
