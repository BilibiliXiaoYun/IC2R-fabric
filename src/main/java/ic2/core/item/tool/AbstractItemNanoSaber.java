...
public abstract class AbstractItemNanoSaber extends ItemElectricTool
    implements ISwingSoundItem, FabricItem {
  ...
  public static boolean isActive(ItemStack stack) {
    return NanoSaberState.isActive(stack);
  }

  private static void setActive(ItemStack stack, boolean active) {
    NanoSaberState.setActive(stack, active);
  }
  ...
  private static void syncActiveState(Player player, ItemStack stack, boolean active) {
    ...
    IC2.network
        .get(true)
        .initiateItemEvent(player, stack, SYNC_ACTIVE_BASE + slot * 2 + (active ? 1 : 0), true);
  }
  ...
  /**
   * NeoForge asked items {@code shouldCauseReequipAnimation(old, new, slotChanged)}; vanilla 1.21.1
   * moved this onto components and Fabric exposes the equivalent as {@code
   * FabricItem#allowComponentsUpdateAnimation}. Suppressing the component-update animation keeps the
   * "only animate when the item itself changed" behaviour, because a slot change is animated by the
   * client independently of this hook.
   */
  @Override
  ...
  /** Replaces NeoForge's {@code shouldCauseBlockBreakReset(old, new) -> false}. */
  @Override
  ...
  public boolean consumeEnergy(ItemStack stack, double amount, LivingEntity entity) {
    if (!super.consumeEnergy(stack, amount, entity)) {
      if (isActive(stack)) {
        ...
        NanoSaberState.setEnergyTick(stack, 0);
  ...
  public boolean canAttackBlock(
      @NotNull BlockState state, @NotNull Level world, @NotNull BlockPos pos, Player miner) {
  ...
  public @NotNull InteractionResultHolder<ItemStack> use(
      @NotNull Level world, Player player, InteractionHand hand) {
  ...
  public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean par5) {
    ...
    int energyTick = NanoSaberState.getEnergyTick(stack) + 1;
    NanoSaberState.setEnergyTick(stack, energyTick);
    ...
    if (event >= SYNC_ACTIVE_BASE) {
      ...
      if (slot < player.getInventory().getContainerSize()) {
        ...
        if (invStack.getItem() instanceof AbstractItemNanoSaber) {
          ...
          if (!active) {
            NanoSaberState.setEnergyTick(invStack, 0);
          }
        }
      }
