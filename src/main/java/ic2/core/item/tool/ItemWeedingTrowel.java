...
public class ItemWeedingTrowel extends Item implements PriorityUsableItem {
    ...
    if (tileEntity instanceof TileEntityCrop tileEntityCrop) {
      if (tileEntityCrop.getCrop() == Crops.weed) {
        StackUtil.dropAsEntity(
            world,
            pos,
            StackUtil.copyWithSize(
                new ItemStack(Ic2Items.WEED), tileEntityCrop.getCurrentAge() + 1));
